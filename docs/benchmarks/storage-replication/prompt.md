...
- This is a homelab. Keep test PVCs small (1-2Gi max)

---

## Objective

Benchmark both the raw VLAN 20 network speed between worker nodes (iperf3) and the effective Longhorn storage replication throughput (fio). Produce a summary that separates network performance from storage performance so the bottleneck is identifiable.
...
2. **Benchmark raw VLAN 20 network speed (iperf3):**
   ...
   - Checkpoint: Output a table of network bandwidth per worker pair. Flag if any pair is below 9 Gbps (expected for 10GbE with overhead).
...
6. **Check replica placement:**
   ...
   - Report replica placement to confirm replication crossed VLAN 20.
...
7. **Produce summary:**
   ...
   - Flag any anomalies (iperf3 retransmits, latency spikes, low throughput on any leg).
...
- Install iperf3 temporarily on worker nodes via SSH for network testing, removed in cleanup
