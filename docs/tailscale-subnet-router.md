...
cluster and Proxmox hosts from any network — **including CGNAT LTE, where inbound WireGuard
cannot work.** The router is **single-homed on VLAN 10 only**; the management plane (VLAN 99) is
intentionally **not** remotely reachable (see below).
...
network that can reach the OPNsense WAN — not from a random LTE hotspot behind CGNAT.

Tailscale solves this because both ends dial **out** to Tailscale's coordination/DERP relays;
no inbound port is required. When direct NAT traversal fails (as it does under double CGNAT),
traffic **relays through DERP** — verified here at ~40–90 ms via DERP(mia), which is fine for
...
|---|---|---|
| Where | Isolated Debian 12 LXC on Proxmox | Keeps Tailscale off the cluster/firewall; clean network story |
...
| Auth identity | **Tagged** node (`tag:subnet-router`) | Tag → ACL-governed and **non-expiring** (tagged devices auto-disable key expiry), so the remote path never silently drops on key expiry. A user-owned login would expire. |
| Reach scope | **Single NIC on VLAN 10**, routes `10.0.10.0/24` only | Directly connected; covers the whole cluster, all Proxmox hosts, and the kube-vip VIP. The **management plane (VLAN 99) is intentionally NOT remotely reachable** — it stays LAN-local by design; remote firewall/switch/JetKVM admin is not a requirement. Keeping the router single-homed also keeps it out of the MGMT segment entirely. |
...
and keeps the appliance conventional.
...
re-applied at boot by `systemd-sysctl`, so it survives reboot — runtime-only was not acceptable):
...
> non-namespaced host sysctls it cannot touch. Our two **namespaced** net values apply cleanly
...
`/var/lib/tailscale`). Joined with a **single-use tagged auth key** (consumed immediately, so it
cannot linger; never written to any file or committed):
...
## ACL policy (operator-maintained — the sole compensating control)
...
> **Sequencing gotcha:** the ACL must be applied **before** minting the tagged auth key —
> Tailscale won't issue a key for a tag that isn't defined in `tagOwners`, and `autoApprovers`
> only auto-approves once the policy is live. Order: apply ACL → generate key → `tailscale up`.

**Key-expiry handling:** the node does not expire because it is **tagged** (Tailscale disables
key expiry on tagged devices automatically). No manual per-node setting is required; confirmed
in the Machines tab.
...
| Check | Result |
|---|---|
| IP forwarding (v4 + v6) inside LXC | ✅ both `= 1`; drop-in applies with rc 0 |
| `/dev/net/tun` present | ✅ `crw-rw-rw- 10, 200` |
| VLAN 10 gateway + internet from LXC | ✅ 0% loss to `10.0.10.1` and `1.1.1.1` |
| Single default route | ✅ only via `eth0`/`10.0.10.1` |
| Node online + tagged + advertising `10.0.10.0/24` | ✅ `BackendState: Running`, `tag:subnet-router`, `[10.0.10.0/24]` |
| Route approved (autoApprovers) | ✅ `tailscale ping` to VLAN 10 IPs resolves **via** the router (unapproved routes would fail) |
| Client joined, accepting routes | ✅ `RedSkelly@github`, `RouteAll: true` |
| Overlay reach → `{10.0.10.49, 10.0.10.10}` | ✅ `tailscale ping` pong via router, ~40–90 ms DERP(mia) |
| VLAN 99 correctly **not** reachable | ✅ `tailscale ping 10.0.99.100` → `no matching peer` (route withdrawn) |
| **Off-LAN acceptance test (LTE)** | ✅ passed 2026-07-22 from LTE: `ping 10.0.10.49`, `ping 10.0.10.10`, `kubectl --context homelab get nodes` (6 Ready) |

---

## Security notes

- **Management plane (VLAN 99) is intentionally NOT remotely reachable.** The router is
  single-homed on VLAN 10; remote firewall/switch/JetKVM administration is not a requirement, so
  the MGMT segment stays LAN-local and the OPNsense VLAN 10 → VLAN 99 segmentation is left fully
  intact. The router is never placed on the MGMT segment.
- The **Tailscale ACL is the sole compensating control** for the cluster's zero NetworkPolicies.
  It restricts the advertised route to `RedSkelly@github` and **replaces the default allow-all**;
  it must **never** be reverted to allow-all.
- The subnet router is a single ingress point into VLAN 10. Treat it as security-sensitive
  infrastructure.
- The node is **tagged and non-expiring by design** so the CGNAT path does not silently drop.
...
- **History:** originally provisioned dual-homed with a second NIC on VLAN 99 (JetKVM/MGMT
  reach). **Collapsed to VLAN 10 only on 2026-07-22** — the management plane stays LAN-local by
...
- **Boot persistence** is complete: `onboot=1` (container autostarts), `tailscaled` enabled
  ...
  reboot-persistence test is recommended as a final confidence check.
