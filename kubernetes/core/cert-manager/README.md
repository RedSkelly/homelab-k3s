...
- No `values.yaml` here: cert-manager's values are chart defaults, inlined as `valuesObject` in the Argo CD Application (`bootstrap/argocd/applications/cert-manager.yaml`)
- Chart: `jetstack/cert-manager` from `https://charts.jetstack.io`
- CRDs are installed with the chart (`crds.enabled: true` in the Application's `valuesObject`)
- Certs auto-renew (90-day duration, renewal ~30 days before expiry)
- ClusterIssuer resources go here alongside the chart values, since they are coupled to cert-manager's lifecycle
